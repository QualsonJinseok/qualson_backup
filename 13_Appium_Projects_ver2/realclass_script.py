...
def run_test_script():
    ...
    time.sleep(1)
    subprocess.Popen("adb shell pm grant com.qualson.realclass android.permission.RECORD_AUDIO", shell=True)
    time.sleep(1)
    subprocess.Popen("adb shell pm grant com.qualson.realclass android.permission.READ_EXTERNAL_STORAGE", shell=True)
    while RUN_COUNT < 4 and SUCCESS_COUNT < 1:
        try:
            ...
            ### 스크립트 ###
            #2. 회원가입 페이지 이동
            id = 'com.qualson.realclass:id/btn_start_register'
            ...
            #3. 이메일 입력 :: xpath_send_keys ==> id_send_keys
            id = 'com.qualson.realclass:id/et_register_email'
            if appium_chk_id_send_keys(id, "email", "이메일 입력") != 200:
                raise Exception
            time.sleep(2)

            #4. 비밀번호 입력
            id = 'com.qualson.realclass:id/et_register_password'
            if appium_chk_id_send_keys(id, "password", "비밀번호 입력") != 200:
                raise Exception
            time.sleep(2)

            #5. 비밀번호 확인용 재입력
            id = 'com.qualson.realclass:id/et_register_password_re'
            if appium_chk_id_send_keys(id, "password", "비밀번호 확인용 재입력") != 200:
                raise Exception
            time.sleep(2)

            #6. 닉네임 입력
            id = 'com.qualson.realclass:id/et_register_nickname'
            if appium_chk_id_send_keys(id, "nickname", "닉네임 입력") != 200:
                raise Exception
            time.sleep(2)

            #7. 휴대전화 인증 페이지로 이동
            id = 'com.qualson.realclass:id/tv_register_next'
            if appium_chk_id_click(id, "휴대전화 인증 페이지로 이동") != 200:
                raise Exception
            time.sleep(2)

            #8. 휴대전화번호 입력
            id = 'com.qualson.realclass:id/et_register_phone_number'
            ...
            ############################################################
            #9. 인증번호 받기 터치
            id = 'com.qualson.realclass:id/btn_register_verification_code'
            if LOOP_SWITCH == True:
                print("start loop")
            ...
            #15. 인증번호 입력        ==> 인증번호가 안맞는 경우, 예외 처리 필요 // 실패 시 == 스크립트 처음부터 재동작
            id = 'com.qualson.realclass:id/et_register_verification_code'
            if appium_chk_id_send_keys(id, "ocr_result", "인증번호 입력") != 200:
                raise Exception
            time.sleep(2)

            #16. 가입 시도
            id = 'com.qualson.realclass:id/tv_register_register'
            ...
            #17. 신규 가입하기 터치
            id = 'com.qualson.realclass:id/tv_dialog_register_register'
            ...
            time.sleep(1)
            ############################################################
            # 아래 구간에서 오류 발생 시, 앱 데이터 날리기
            ############################################################
            #18. 로그인 완료 팝업의 확인버튼 터치
            id = 'com.qualson.realclass:id/tv_dialog_no_title_one_button_confirm'
            if appium_chk_id_click(id, "로그인 완료 팝업의 확인버튼 터치") != 200:
                LOGIN_CHECK = True
                raise Exception
            time.sleep(2)

            #19. 하단 카테고리 계정 선택
            id = 'com.qualson.realclass:id/iv_profile'
            if appium_chk_id_click(id, "하단 카테고리 계정 선택") != 200:
                LOGIN_CHECK = True
                raise Exception
            time.sleep(2)

            #20. 설정 버튼 터치
            id = 'com.qualson.realclass:id/view_account_setting'
            if appium_chk_id_click(id, "설정 버튼 터치") != 200:
                LOGIN_CHECK = True
                raise Exception
            time.sleep(2)
            ...
            #22. 로그아웃 터치
            id = 'com.qualson.realclass:id/tv_settings_logout'
            if appium_chk_id_click(id, "로그아웃 버튼 터치") != 200:
                LOGIN_CHECK = True
                raise Exception
            time.sleep(2)

            #23. 로그아웃 확인 버튼 터치
            id = 'com.qualson.realclass:id/tv_dialog_title_two_button_positive'
            ...
            if SUCCESS_COUNT == 1:
                ...
                if LOGIN_CHECK == True:
                    subprocess.Popen("adb shell pm clear com.qualson.realclass", shell=True)
                    time.sleep(1)
                    subprocess.Popen("adb shell pm grant com.qualson.realclass android.permission.READ_EXTERNAL_STORAGE", shell=True)
                    time.sleep(1)
                    subprocess.Popen("adb shell pm grant com.qualson.realclass android.permission.RECORD_AUDIO", shell=True)
